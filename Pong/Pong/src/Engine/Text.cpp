...
{
	if (font == nullptr) {
		std::cout << "ERROR - The font with " << asset_id << " was not loaded correctly and is a nullptr." << std::endl;
		return;
	}
	...
	if (surface == NULL) {
		std::cout << "ERROR - A problem occured when loading a surface from a font with "
			<< asset_id << " | " << SDL_GetError() << std::endl;
		return;
	}
	...
	if (texture == NULL) {
		std::cout << "ERROR - A problem occured when loading a texture from a surface in fonts with "
			<< asset_id << " | " << SDL_GetError() << std::endl;
		return;
	}
